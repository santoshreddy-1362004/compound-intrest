...
export default function Home() {
  ...
  const handleCalculate = async ({ amount, days }) => {
    ...
    try {
      // Get live APR from Compound III
      const { getSupplyRate } = require('../utils/compound');
      console.log(`🔄 Calculating for token: ${token}, amount: ${amount}, days: ${days}`);
      const apr = await getSupplyRate(token); // ✅ Real-time rate from Compound

      console.log(`✅ Successfully fetched ${token} rate: ${apr.toFixed(3)}%`);
      ...
      setResult({ 
        ...
        days, 
        finalBalance, 
        token,  // Make sure the current token is used
        apr: apr.toFixed(3) + '% (Real Compound V3)',
        isRealData: true 
      });
      setChartData(data);
    } catch (error) {
